...
# ---------------- LOGGING ----------------
logging.basicConfig(
    filename="system_log_SIM_30.txt",
    level=logging.DEBUG,
...
flush_handler = FlushFileHandler("system_log_SIM_30.txt")
...
state = {
    ...
    "log_msg": "Sistem Baslatiliyor (Drone 30)...",
    "pid_h": 0.0,
    "pid_v": 0.0
}
...
def udp_listener():
    ...
    sock.settimeout(1.0)
    print(f"Drone 20 Dinleniyor (UDP): {LISTEN_PORT}")
...
def main():
    print("Sistem Basladi (Drone 30). Veriler bekleniyor...")
    ...
    while True:
        ...
        with lock:
            ...
            L_hdg = state["leader_heading"]  # Drone 20'nin Yönü
        ...
        if have_leader and have_follower:
            ...
            # FAZLA YAKLAŞINCA ÇALIŞACAK KAÇIŞ VE ÇARPIŞMA ÖNELME
            ...
            if dist_to_target > YAW_ALIGN_THRESH:
                ...
                # atan2(x,y) -> Navigasyon açısı verir (Doğu, Kuzey)
            ...
            # Loglama
            dist_real = math.hypot(dx_real, dy_real)  # Lidere gerçek uzaklık
            with lock:
            ...
            with lock:
                state["log_msg"] = "Drone 20 verisi bekleniyor..."
                state["pid_h"] = 0.0
